...
void Convert :: ipl2momvop(const Mat& src, MomVop* dst)
{
    assert( !src.empty() );
 ...
 */
void Convert :: setShape(const Mat& mask, MomVop* img, int val, int fval, Rect* roi )
{
    assert( !mask.empty() );
 ...
 * note: image sizes should be compatible (not checked)
 */
void Convert :: setShape(const Mat& mask, MomVop* img, int val, int bval, int fval, Rect* roi )
{
    assert( !mask.empty() );
	assert( img );	// none should be NULL or abort will be called
