...
Frame :: Frame(const Mat& src, bool imgFlag, bool grayFlag, bool maskFlag )
{
    this->image = 0;
    this->gray = 0;
    this->mask = 0;
...
void Frame :: setImage(const Mat& src )
{
    if(this->image && !src.empty())
        Convert::ipl2momvop( src, this->image );
}

void Frame :: setGray(const Mat& gray )
{
...
void Frame :: setMask(const Mat& mask, int rid, int fg, int bg )
{
...
void Frame :: setMaskAll(const Mat& mask, int rid, int fg, int bg)
{
    if(!mask.empty())
...
void Frame :: setMaskImage(const Mat& mask, int rid, int fg, int bg)
{
    if(!mask.empty())
        this->setMask( mask, rid, fg, bg);
...
void Frame :: setMaskGray(const Mat& mask, int rid, int fg, int bg)
{
    if(!mask.empty())
        this->setMask( mask, rid, fg, bg);
